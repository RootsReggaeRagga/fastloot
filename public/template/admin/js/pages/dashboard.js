$(function () {
    function t(t, e) {
        function a() {
            i = n.node().getBoundingClientRect().width - r.left - r.right, v.attr("width", i + r.left + r.right), x.attr("width", i + r.left + r.right), u.range([0, i]), x.selectAll(".d3-axis-horizontal").call(m), x.selectAll(".d3-axis-subticks").attr("x1", u).attr("x2", u), x.selectAll(".d3-grid-dashed").call(y.tickSize(-i, 0, 0)), x.selectAll(".d3-axis-right").attr("transform", "translate(" + i + ", 0)"), x.selectAll(".streamgraph-layer").attr("d", function (t) {
                return k(t.values)
            })
        }
        ...
        var n = d3.select(t), r = {
            ...
            bottom: 40,
            left: 50
        }, i = n.node().getBoundingClientRect().width - r.left - r.right, e = e - r.top - r.bottom, s = 30, l = n.append("div").attr("class", "d3-tip e").style("display", "none"), o = d3.time.format("%m/%d/%y %H:%M"), d = d3.time.format("%H:%M"), c = ["#03A9F4", "#29B6F6", "#4FC3F7", "#81D4FA", "#B3E5FC", "#E1F5FE"], u = d3.time.scale().range([0, i]), p = d3.scale.linear().range([e, 0]), f = d3.scale.ordinal().range(c), m = d3.svg.axis().scale(u).orient("bottom").ticks(d3.time.hours, 4).innerTickSize(4).tickPadding(8).tickFormat(d3.time.format("%H:%M")), h = d3.svg.axis().scale(p).ticks(6).innerTickSize(4).outerTickSize(0).tickPadding(8).tickFormat(function (t) {
            return t / 1e3 + "k"
        }), g = h, y = d3.svg.axis().scale(p).orient("left").ticks(6).tickPadding(8).tickFormat("").tickSize(-i, 0, 0), v = n.append("svg"), x = v.attr("width", i + r.left + r.right).attr("height", e + r.top + r.bottom).append("g").attr("transform", "translate(" + r.left + "," + r.top + ")"), b = d3.layout.stack().offset("silhouette").values(function (t) {
            return t.values
        }).x(function (t) {
            return t.date
        }).y(function (t) {
        ...
        d3.csv("assets/demo_data/dashboard/traffic_sources.csv", function (a, c) {
            ...
            var A = x.append("g").attr("class", "streamgraph-layers-group"), z = A.selectAll(".streamgraph-layer").data(v).enter().append("path").attr("class", "streamgraph-layer").attr("d", function (t) {
                return k(t.values)
            ...
            x.append("g").attr("class", "d3-axis d3-axis-left d3-axis-solid").call(h.orient("left")), d3.select(x.selectAll(".d3-axis-left .tick text")[0][0]).style("visibility", "hidden"), x.append("g").attr("class", "d3-axis d3-axis-right d3-axis-solid").attr("transform", "translate(" + i + ", 0)").call(g.orient("right")), d3.select(x.selectAll(".d3-axis-right .tick text")[0][0]).style("visibility", "hidden");
            var M = x.append("g").attr("class", "d3-axis d3-axis-horizontal d3-axis-solid").attr("transform", "translate(0," + e + ")").call(m);
            ...
            var B = A.append("g").attr("class", "hover-line"), F = B.append("line").attr("y1", 0).attr("y2", e).style("fill", "none").style("stroke", "#fff").style("stroke-width", 1).style("pointer-events", "none").style("shape-rendering", "crispEdges").style("opacity", 0), S = B.append("rect").attr("x", 2).attr("y", 2).attr("width", 6).attr("height", 6).style("fill", "#03A9F4").style("stroke", "#fff").style("stroke-width", 1).style("shape-rendering", "crispEdges").style("pointer-events", "none").style("opacity", 0);
            C.each("end", function () {
    ...
    function e(t, e) {
        ...
        function n() {
            ...
            var a = d3.nest().key(function (t) {
                return t.type
            }).map(formatted), n = f.val(), d = a[n];
            c.domain(d3.keys(d[0]).filter(function (t) {
                return "date" !== t && "type" !== t
            }));
            var m = c.domain().map(function (t) {
                return {
                    name: t, values: d.map(function (e) {
                        return {name: t, date: o(e.date), value: parseFloat(e[t], 10)}
                    })
                }
                ...
                return y(t.value)
            }).interpolate("cardinal"), g = d3.time.scale().domain([d3.min(m, function (t) {
            ...
            })]).range([0, s]), y = d3.scale.linear().domain([d3.min(m, function (t) {
                return d3.min(t.values, function (t) {
                    return t.value
                })
            }), d3.max(m, function (t) {
            ...
            })]).range([e, 0]), v = d3.svg.axis().scale(g).orient("bottom").tickPadding(8).ticks(d3.time.days).innerTickSize(4).tickFormat(d3.time.format("%a")), x = d3.svg.axis().scale(y).orient("left").ticks(6).tickSize(0 - s).tickPadding(8);
            ...
            var b = p.selectAll(".lines").data(m), w = b.enter().append("g").attr("class", "lines").attr("id", function (t) {
                return t.name + "-line"
            });
        ...
        var r = d3.select(t), i = {
            ...
            bottom: 30,
            left: 50
        }, s = r.node().getBoundingClientRect().width - i.left - i.right, e = e - i.top - i.bottom, l = d3.tip().attr("class", "d3-tip").html(function (t) {
        ...
        }), o = d3.time.format("%Y/%m/%d").parse, d = (d3.time.format("%b %d, '%y"), ["#4CAF50", "#FF5722", "#5C6BC0"]), c = d3.scale.ordinal().range(d), u = r.append("svg"), p = u.attr("width", s + i.left + i.right).attr("height", e + i.top + i.bottom).append("g").attr("transform", "translate(" + i.left + "," + i.top + ")").call(l), f = $("#select_date").multiselect({
        ...
        $(".multiselect-container input").uniform({radioClass: "choice"}), d3.csv("assets/demo_data/dashboard/app_sales.csv", function (t, e) {
            formatted = e, n()
        });
        var m;
    ...
    function a() {
        d3.csv("assets/demo_data/dashboard/app_sales_heatmap.csv", function (t, e) {
            ...
            var n = d3.nest().key(function (t) {
                return t.app
            }), r = n.entries(e), i = d3.time.format("%Y/%m/%d %H:%M"), s = d3.time.format("%H:%M");
            ...
            var o = d3.time.scale().range([0, width]), d = d3.scale.linear().range([height, 0]), c = d3.scale.quantile().domain([0, buckets - 1, d3.max(e, function (t) {
                return t.value
            })]).range(colors);
            o.domain([new Date(e[0].date), d3.time.hour.offset(new Date(e[e.length - 1].date), 1)]), d.domain([0, d3.max(e, function (t) {
                return t.app
            })]);
            var u = l.append("svg"), p = u.attr("width", width + margin.left + margin.right).attr("height", height + margin.bottom).append("g").attr("transform", "translate(" + margin.left + "," + margin.top + ")"), f = p.selectAll(".hour-group").data(r).enter().append("g").attr("class", "hour-group").attr("transform", function (t, e) {
                return "translate(0, " + (gridSize + rowGap) * e + ")"
            });
            ...
            var m = f.selectAll(".heatmap-hour").data(function (t) {
                return t.values
            ...
            m.transition().duration(250).delay(function (t, e) {
                return 20 * e
            ...
            e.forEach(function (t, a) {
                g = d3.max(e, function (t) {
                    return t.value
                }), h = d3.min(e, function (t) {
                    return t.value
                })
            });
            var y = p.append("g").attr("class", "legend-group").attr("width", width).attr("transform", "translate(" + (width / 2 - buckets * gridSize / 2) + "," + (height + (margin.bottom - margin.top)) + ")"), v = y.selectAll(".heatmap-legend").data([0].concat(c.quantiles()), function (t) {
                return t
            ...
            v.append("rect").attr("class", "heatmap-legend-item").attr("x", function (t, e) {
                ...
                return colors[e]
            }), y.append("text").attr("class", "min-legend-value").attr("x", -10).attr("y", -2).style("text-anchor", "end").style("font-size", 11).style("fill", "#999").text(h), y.append("text").attr("class", "max-legend-value").attr("x", buckets * gridSize + 10).attr("y", -2).style("font-size", 11).style("fill", "#999").text(g), $(window).on("resize", a), $(document).on("click", ".sidebar-control", a)
        })
    }
    ...
    function n(t, e, a) {
        var n = d3.select(t), r = {
            ...
            bottom: 40,
            left: 35
        }, i = n.node().getBoundingClientRect().width - r.left - r.right, e = e - r.top - r.bottom, s = d3.time.format("%Y-%m-%d").parse, l = d3.bisector(function (t) {
            return t.date
        }).left, o = d3.time.format("%b %d"), d = n.append("svg"), c = d.attr("width", i + r.left + r.right).attr("height", e + r.top + r.bottom).append("g").attr("transform", "translate(" + r.left + "," + r.top + ")"), u = d3.svg.area().x(function (t) {
            return p(t.date)
        }).y0(e).y1(function (t) {
            return f(t.value)
        }).interpolate("monotone"), p = d3.time.scale().range([0, i]), f = d3.scale.linear().range([e, 0]), m = d3.svg.axis().scale(p).orient("bottom").ticks(d3.time.days, 6).innerTickSize(4).tickPadding(8).tickFormat(d3.time.format("%b %d"));
        d3.json("assets/demo_data/dashboard/monthly_sales.json", function (t, h) {
            function g() {
                var t = d3.mouse(this), a = t[0], i = (t[1], p.invert(a)), s = l(h, i), d = h[s - 1], c = h[s], u = i - d.date > c.date - i ? c : d;
            ...
            function y() {
                i = n.node().getBoundingClientRect().width - r.left - r.right, d.attr("width", i + r.left + r.right), c.attr("width", i + r.left + r.right), p.range([0, i]), c.selectAll(".d3-axis-horizontal").call(m), c.selectAll(".d3-axis-subticks").attr("x1", p).attr("x2", p), c.selectAll(".d3-area").datum(h).attr("d", u), c.selectAll(".d3-crosshair-overlay").attr("width", i)
            }
    ...
    function r(t, e, a) {
        var n = d3.select(t), r = {
            ...
            bottom: 0,
            left: 0
        }, i = n.node().getBoundingClientRect().width - r.left - r.right, e = e - r.top - r.bottom, s = d3.time.format("%Y-%m-%d").parse, l = n.append("svg"), o = l.attr("width", i + r.left + r.right).attr("height", e + r.top + r.bottom).append("g").attr("transform", "translate(" + r.left + "," + r.top + ")"), d = d3.svg.area().x(function (t) {
            return c(t.date)
        }).y0(e).y1(function (t) {
            return u(t.value)
        }).interpolate("monotone"), c = d3.time.scale().range([0, i]), u = d3.scale.linear().range([e, 0]);
    ...
    function i(t, e, a, n, r, i, s, l) {
        function o() {
            A.attr("transform", null).transition().duration(i).ease("linear").attr("transform", "translate(" + h(0) + ",0)"), "area" == e ? A.attr("d", v).attr("class", "d3-area").style("fill", l) : A.attr("d", y).attr("class", "d3-line d3-line-medium").style("stroke", l)
        }
        ...
        var h = d3.scale.linear().range([0, p]), g = d3.scale.linear().range([n - 5, 5]);
        ...
        var y = d3.svg.line().interpolate(r).x(function (t, e) {
            ...
            return g(t)
        }), x = c.append("svg"), b = x.attr("width", p + u.left + u.right).attr("height", n + u.top + u.bottom).append("g").attr("transform", "translate(" + u.left + "," + u.top + ")"), w = b.append("defs").append("clipPath").attr("id", function (e, a) {
    ...
    function s(t, e) {
        function a() {
            s = r.node().getBoundingClientRect().width - i.left - i.right, u.attr("width", s + i.left + i.right), p.attr("width", s + i.left + i.right), f.range([l, s - l]), y.attr("width", s), p.selectAll(".d3-line").attr("d", h(n)), p.selectAll(".d3-line-circle").attr("cx", h.x()), p.selectAll(".d3-line-guides").attr("x1", function (t, e) {
                return f(t.date)
        ...
        var n = [{date: "04/13/14", alpha: "60"}, {date: "04/14/14", alpha: "35"}, {
            date: "04/15/14",
            alpha: "65"
        }, {date: "04/16/14", alpha: "50"}, {date: "04/17/14", alpha: "65"}, {
            date: "04/18/14",
            alpha: "20"
        }, {date: "04/19/14", alpha: "60"}], r = d3.select(t), i = {
            top: 0,
            right: 0,
            bottom: 0,
            left: 0
        }, s = r.node().getBoundingClientRect().width - i.left - i.right, e = e - i.top - i.bottom, l = 20, o = d3.time.format("%m/%d/%y").parse, d = d3.time.format("%a, %B %e"), c = d3.tip().attr("class", "d3-tip").html(function (t) {
        ...
        }), u = r.append("svg"), p = u.attr("width", s + i.left + i.right).attr("height", e + i.top + i.bottom).append("g").attr("transform", "translate(" + i.left + "," + i.top + ")").call(c);
        n.forEach(function (t) {
            t.date = o(t.date), t.alpha = +t.alpha
        });
        var f = d3.time.scale().range([l, s - l]), m = d3.scale.linear().range([e, 5]);
        ...
        var h = d3.svg.line().x(function (t) {
            ...
            return m(t.alpha)
        }), g = p.append("defs").append("clipPath").attr("id", "clip-line-small"), y = g.append("rect").attr("class", "clip").attr("width", 0).attr("height", e);
        ...
        var x = p.insert("g").selectAll(".d3-line-circle").data(n).enter().append("circle").attr("class", "d3-line-circle d3-line-circle-medium").attr("cx", h.x()).attr("cy", h.y()).attr("r", 3).style("stroke", "#fff").style("fill", "#29B6F6");
    ...
    function l(t, e, a, n) {
        function r() {
            ...
            return function (e) {
                var a = d / (100 / e), n = c.endAngle(o * a);
                return c(t(n))
            }
        }

        var i = d3.select(t), s = 2, l = Math.min(e / 2, a / 2) - s, o = 2 * Math.PI, d = $(t).data("progress"), c = d3.svg.arc().startAngle(0).innerRadius(0).outerRadius(l).endAngle(function (t) {
            return t.value / t.size * 2 * Math.PI
        }), u = i.append("svg"), p = u.attr("width", e).attr("height", a).append("g").attr("transform", "translate(" + e / 2 + "," + a / 2 + ")"), f = p.append("g").attr("class", "progress-meter");
    ...
    function o(t, e) {
        var a = [{
            ...
            value: 3909,
            color: "#FF7043"
        }], n = d3.select(t), r = 2, i = e / 2 - r, s = d3.sum(a, function (t) {
            return t.value
        ...
        }), o = n.append("svg").call(l), d = o.attr("width", e).attr("height", e).append("g").attr("transform", "translate(" + e / 2 + "," + e / 2 + ")"), c = d3.layout.pie().sort(null).startAngle(Math.PI).endAngle(3 * Math.PI).value(function (t) {
            return t.value
        }), u = d3.svg.arc().outerRadius(i).innerRadius(i / 2), p = d.selectAll(".d3-arc").data(c(a)).enter().append("g").attr("class", "d3-arc").style("stroke", "#fff").style("cursor", "pointer"), f = p.append("path").style("fill", function (t) {
            return t.data.color
        });
        f.on("mouseover", function (t, e) {
            d3.select(this).transition().duration(500).ease("elastic").attr("transform", function (t) {
                ...
                var e = Math.sin(t.midAngle) * r, a = -Math.cos(t.midAngle) * r;
    ...
    function u(t, e, a, n, r, i, s, l, o) {
        ...
        var m = d3.select(t), h = m.node().getBoundingClientRect().width, g = d3.scale.ordinal().rangeBands([0, h], .3), y = d3.scale.linear().range([0, a]);
        g.domain(d3.range(0, p.length)), y.domain([0, d3.max(p)]);
        var v = m.append("svg"), x = v.attr("width", h).attr("height", a).append("g"), b = x.selectAll("rect").data(p).enter().append("rect").attr("class", "d3-random-bars").attr("width", g.rangeBand()).attr("x", function (t, e) {
            return g(e)
        }).style("fill", l), w = d3.tip().attr("class", "d3-tip").offset([-10, 0]);
    ...
    function p(t, e, a, n, r, i, s, l) {
        ...
        var d = d3.select(t), c = 0, u = 32, p = r, f = 2 * Math.PI, m = d3.format(".0%"), h = 2 * e, g = Math.abs((p - c) / .01), y = c > p ? -.01 : .01, v = d.append("svg"), x = v.attr("width", h).attr("height", h).append("g").attr("transform", "translate(" + h / 2 + "," + h / 2 + ")"), b = d3.svg.arc().startAngle(0).innerRadius(e).outerRadius(e - a);
        x.append("path").attr("class", "d3-progress-background").attr("d", b.endAngle(f)).style("fill", "#eee");
        var w = x.append("path").attr("class", "d3-progress-foreground").attr("filter", "url(#blur)").style("fill", n).style("stroke", n), k = x.append("path").attr("class", "d3-progress-front").style("fill", n).style("fill-opacity", 1), A = d3.select(t).append("h2").attr("class", "mt-15 mb-5");
        d3.select(t).append("i").attr("class", i + " counter-icon").attr("style", "top: " + (h - u) / 2 + "px"), d3.select(t).append("div").text(s), d3.select(t).append("div").attr("class", "text-size-small text-muted").text(l);
        var z = c;
        !function C() {
    ...
    function f(t, e) {
            ...
            d3.bullet = function () {
                function i(t) {
                    t.each(function (t, e) {
                        ...
                        var i = u.call(this, t, e).slice().sort(d3.descending), o = p.call(this, t, e).slice().sort(d3.descending), g = f.call(this, t, e).slice().sort(d3.descending), y = d3.select(this), v = d3.scale.linear().domain([0, Math.max(i[0], o[0], g[0])]).range(d ? [l, 0] : [0, l]), x = this.__chart__ || d3.scale.linear().domain([0, 1 / 0]).range(v.range());
                        this.__chart__ = v;
                        var b = r(x), w = r(v), k = y.selectAll(".bullet-range").data(i);
                        k.enter().append("rect").attr("class", function (t, e) {
                            return "bullet-range bullet-range-" + (e + 1)
                        }).attr("width", b).attr("height", m).attr("rx", 2).attr("x", d ? x : 0).transition().duration(c).attr("width", w).attr("x", d ? v : 0), k.transition().duration(c).attr("x", d ? v : 0).attr("width", w).attr("height", m);
                        ...
                        }).attr("width", b).attr("height", m / 5).attr("x", d ? x : 0).attr("y", m / 2.5).style("shape-rendering", "crispEdges"), A.transition().duration(c).attr("width", w).attr("x", d ? v : 0), A.transition().duration(c).attr("width", w).attr("height", m / 5).attr("x", d ? v : 0).attr("y", m / 2.5);
                        var z = y.selectAll(".bullet-marker").data(o);
                        z.enter().append("line").attr("class", function (t, e) {
                            return "bullet-marker bullet-marker-" + (e + 1)
                        }).attr("x1", x).attr("x2", x).attr("y1", m / 6).attr("y2", 5 * m / 6), z.transition().duration(c).attr("x1", v).attr("x2", v), z.transition().duration(c).attr("x1", v).attr("x2", v).attr("y1", m / 6).attr("y2", 5 * m / 6);
                        var C = h || v.tickFormat(8), M = y.selectAll(".bullet-tick").data(v.ticks(8), function (t) {
                            return this.textContent || C(t)
                        }), B = M.enter().append("g").attr("class", "bullet-tick").attr("transform", n(x)).style("opacity", 1e-6);
                        B.append("line").attr("y1", m).attr("y2", 7 * m / 6 + 3), B.append("text").attr("text-anchor", "middle").attr("dy", "1em").attr("y", 7 * m / 6 + 4).text(C), B.transition().duration(c).attr("transform", n(v)).style("opacity", 1);
                        var F = M.transition().duration(c).attr("transform", n(v)).style("opacity", 1);
                        F.select("line").attr("y1", m + 3).attr("y2", 7 * m / 6 + 3), F.select("text").attr("y", 7 * m / 6 + 4), M.exit().transition().duration(c).attr("transform", n(v)).style("opacity", 1e-6).remove(), $(window).on("resize", a), $(document).on("click", ".sidebar-control", a)
                    }), d3.timer.flush()
                }

                var o = "left", d = !1, c = 750, u = t, p = e, f = a, m = 30, h = null;
        ...
        var i = d3.select(t), s = {
            ...
            bottom: 35,
            left: 10
        }, l = l = i.node().getBoundingClientRect().width - s.left - s.right, e = e - s.top - s.bottom, o = d3.bullet().width(l).height(e);
        d3.json("assets/demo_data/dashboard/bullets.json", function (t, a) {
            ...
            if (t)return console.error(t);
            var d = i.selectAll("svg").data(a).enter().append("svg"), c = d.attr("class", function (t, e) {
                return "bullet-" + (e + 1)
            }).attr("width", l + s.left + s.right).attr("height", e + s.top + s.bottom).append("g").attr("transform", "translate(" + s.left + "," + s.top + ")").call(o), u = c.append("g").style("text-anchor", "start");
            ...
            var m = document.querySelector(".switcher");
            new Switchery(m);
            m.onchange = function () {
                if (m.checked)f.push(setInterval(function () {
                    p()
                }, 5e3)); else for (var t = 0; t < f.length; t++)clearInterval(f[t])
            }, $(window).on("resize", r), $(document).on("click", ".sidebar-control", r)
        })
    }

    var m = Array.prototype.slice.call(document.querySelectorAll(".switch"));
    m.forEach(function (t) {
        new Switchery(t, {color: "#4CAF50"})
        ...
        var e = $(this).find(".letter-icon-title"), a = e.eq(0).text().charAt(0).toUpperCase(), n = $(this).find(".letter-icon");
        n.eq(0).text(a)
    })
});