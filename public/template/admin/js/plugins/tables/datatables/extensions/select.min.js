...
*/
(function(e){"function"===typeof define&&define.amd?define(["jquery","http://demo.interface.club/limitless/layout_4/LTR/default/assets/js/plugins/tables/datatables/extensions/datatables.net"],function(j){return e(j,window,document)}):"object"===typeof exports?module.exports=function(j,l){j||(j=window);if(!l||!l.fn.dataTable)l=require("http://demo.interface.club/limitless/layout_4/LTR/default/assets/js/plugins/tables/datatables/extensions/datatables.net")(j,l).$;return e(l,j,j.document)}:e(jQuery,window,document)})(function(e,j,l,i){function s(a){var b=a.settings()[0]._select.selector;e(a.table().body()).off("mousedown.dtSelect",b).off("mouseup.dtSelect",b).off("click.dtSelect",b);e("body").off("click.dtSelect")}
...
i}),a=c.cells({selected:!0}).eq(0).map(function(b){var a=c.row(b.row).id(!0);return a?{row:a,column:b.column}:i}).filter(function(b){return b!==i});c.one("draw.dt.dtSelect",function(){c.rows(b).select();a.any()&&a.each(function(b){c.cells(b.row,b.column).select()})})});c.on("http://demo.interface.club/limitless/layout_4/LTR/default/assets/js/plugins/tables/datatables/extensions/draw.dtSelect.dt select.dtSelect.dt deselect.dtSelect.dt",function(){v(c)});c.on("destroy.dtSelect",function(){s(c);c.off(".dtSelect")})}var d=new h.Api(b);s(d);"api"!==a&&u(d);k(new h.Api(b),"selectStyle",[a])})});o("select.selector()",
